...
public class AppTest extends BaseTest {
    ...
    public void testOnlinePayCardPlaceholders() {
        ...
        new WebDriverWait(getDriver(), Duration.ofSeconds(10))
                .until(ExpectedConditions.visibilityOfElementLocated
                        (By.xpath("//app-card-input")));
        SoftAssert softAssert = new SoftAssert();
        softAssert.assertEquals(getDriver().findElement(By.xpath("//input[@id='cc-number']/following-sibling::label")).getText(), "Номер карты");
        softAssert.assertEquals(getDriver().findElement(By.xpath("//input[@autocomplete='cc-exp']/following-sibling::label")).getText(), "Срок действия");
        softAssert.assertEquals(getDriver().findElement(By.xpath("//input[@autocomplete='cc-csc']/following-sibling::label")).getText(), "CVC");
        softAssert.assertEquals(getDriver().findElement(By.xpath("//input[@autocomplete='cc-name']/following-sibling::label")).getText(), "Имя держателя (как на карте)");
        softAssert.assertAll();
    }
    ...
    public void testOnlinePayPaySystems(String paySystemLogoAdr) {
        addPaymentData();
        WebElement paySystemLogo = new WebDriverWait(getDriver(), Duration.ofSeconds(10))
                .until(ExpectedConditions.visibilityOfElementLocated
